...
const AllFoods = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const { data } = await axios(
        ...
        }/all-foods?page=${currentPage}&size=${itemsPerPage}&filter=${filter}&sort=${sort}&search=${search}`
      );
      setFoods(data);
    };
    getData();
  ...
  useEffect(() => {
    const getCount = async () => {
      try {
        const { data } = await axios(
          `${import.meta.env.VITE_API_URL}/foods-count?filter=${filter}&search=${search}`
        );
        setCount(data.count);
      } catch (error) {
